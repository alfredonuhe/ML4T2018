...
class DTLearner(object):  		   	  			    		  		  		    	 		 		   		 		  
    ...
    def addEvidence(self,dataX,dataY):  		   	  			    		  		  		    	 		 		   		 		  
        ...
        if np.allclose(dataY, dataY[0]): return np.array([-1, dataY[0], -1, -1])
            ...
            # i is the second maximun value of the last row in the correlation matrix
            # absolute correlation values are sorted and the second to last is selected
            i = np.nanargmax(np.absolute(np.corrcoef(dataX, y=dataY, rowvar=False)[-1,:-1]))
